...
# Height above the bed that a "tap" probe is started from. docs/Eddy_Probe.md
# requires 3-20mm; starting closer risks contact going undetected.
TAP_START_Z = 5.
...
class TemperatureProbe:
    ...
    def _manual_probe_finalize(self, mpresult):
        ...
        # Use the probe result (not the live toolhead position) as the
        # bed reference. With METHOD=manual the two coincide, but
        # METHOD=tap ends with a deliberate pullback move, so the
        # toolhead is not resting at the bed height.
    ...
    def cmd_TEMPERATURE_PROBE_NEXT(self, gcmd):
        ...
        self.gcode.register_command("ABORT", None)
        if self._method == "tap":
            # Do not return to the resting height. It is only resting_z above
            # the bed, which is far too close to begin a tap from.
            curpos[2] = self.last_zero_pos + TAP_START_Z
            toolhead.manual_move(curpos, lift_speed)
            self._auto_probe(gcmd)
            return
        curpos[2] = start_z
